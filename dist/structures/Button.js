...
class Button {
    ...
    setStyle(style) {
        if (typeof this._raw.label === `string` && style === ButtonStyle.LINK)
            throw new Error(`Cannot use style "LINK" when the button has a "label" property assigned`);
        if (typeof this._raw.url === `string` && style !== ButtonStyle.LINK)
            throw new Error(`Cannot use style "${ButtonStyle[style]}" when the button has a "url" property assigned`);
    ...
    setId(id) {
        if (this._raw.style === DiscordTypes.ButtonStyle.Link)
            throw new Error(`Cannot set custom_id when the button has the "LINK" style`);
    ...
    setURL(url) {
        if (this._raw.style !== undefined && this._raw.style !== DiscordTypes.ButtonStyle.Link)
            throw new Error(`Cannot set URL when the button has the "${ButtonStyle[this._raw.style]}" style`);
        this._raw.url = url;
        return this;
    }
    /**
    ...
    getRaw() {
        if (typeof this._raw.style !== `number`)
            throw new Error(`Cannot convert a button with a missing "style" parameter to raw`);
        if (typeof this._raw.custom_id !== `string` && typeof this._raw.url !== `string`)
            throw new Error(`Cannot convert a button with a missing "custom_id" or "url" parameter to raw`);
        if (typeof this._raw.custom_id === `string` && typeof this._raw.url === `string`)
            throw new Error(`Cannot convert a button with both "custom_id" and "url" parameters defined to raw`);
        if (this._raw.custom_id !== undefined && this._raw.style === DiscordTypes.ButtonStyle.Link)
            throw new Error(`Cannot convert a button to raw when the button has the "LINK" style with a "custom_id" parameter`);
        if (this._raw.url !== undefined && this._raw.style !== DiscordTypes.ButtonStyle.Link)
            throw new Error(`Cannot convert a button to raw when the button has the "${ButtonStyle[this._raw.style]}" style with a "url" parameter`);
        return {
