...
> = Modal<Fields & { [key in K]: R extends true ? T : T | undefined }>;
...
export class Modal<Fields extends Record<string, any> = Record<string, never>> {
   ...
   * @returns The modal.
   */
  public addTextField<R extends boolean, K extends string>(
    required: R,
    id: K,
    label: string,
    style: ModalTextFieldStyle,
    options?: { minLength?: number; maxLength?: number; placeholder?: string },
   ...
   */
  public setExecute(
    executeCallback: (ctx: ModalContext<Fields>) => void | Promise<void>,
  ): this {
...
export class ModalContext<
  Fields extends Record<string, any> = Record<string, never>,
> extends InteractionContext {
   ...
   */
  constructor(
    interaction: DiscordTypes.APIModalSubmitInteraction,
