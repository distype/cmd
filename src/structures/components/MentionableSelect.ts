...
export class MentionableSelect extends BaseSelect<DiscordTypes.ComponentType.MentionableSelect> {
    declare setExecute: (executeCallback: BaseComponentContextCallback<MentionableSelectContext>) => this;
...
export class MentionableSelectContext extends BaseComponentContext {
     ...
     */
    public readonly options: Array<{ user: DiscordTypes.APIUser, member?: DiscordTypes.APIInteractionDataResolvedGuildMember } | DiscordTypes.APIRole>;
    ...
    constructor (interaction: SelectInteraction<DiscordTypes.ComponentType.MentionableSelect>, commandHandler: CommandHandler) {
        ...
        this.options = [
            ...(interaction.data.resolved.users ? Object.keys(interaction.data.resolved.users).map((id) => ({
